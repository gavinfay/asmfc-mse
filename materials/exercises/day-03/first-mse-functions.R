...
projection.plot <- function(project.results) {
  Fig2 <- ggplot(data = subset(project.results, type != "index"),
                 aes(x = year, y = value))
  Fig2 + stat_summary(fun.data = "median_hilow", geom = "ribbon", 
                          fill=gray(0.6),alpha=0.3) +
    stat_summary(fun.data = "median_hilow", geom="line",lwd=1) +
    geom_line(aes(y=value,x=year,group=iter),data = subset(project.results, type != "index" & iter <10 & year %in% proj.years), col=gray(0.7)) +
    facet_wrap(~type, scale = "free_y") + ylab("Million tonnes") + theme_bw()
}
